...
% Second layer is the convolution layer - CeNN convolution op is easy
layers{2}.type = 'CONV';
%layers{2}.num = 20; % # of output channels
layers{2}.k = 3; % kernel size in CeNN is 3X3
%layers{2}.stride = 1; % stride size
%layers{2}.pad = 0; % padding size
layers{2}.group = 1; % group of input feature maps
layers{2}.Btemplate = zeros(3,3);
layers{2}.threshold = 0;

% Third layer is the ReLU layer
layers{3}.type = 'RELU';

% Fourth layer is the pooling layer
...
% Fifth layer is  fully-connected inner product layer
layers{5}.type = 'IP';
layers{5}.num = 500; % # of output dimension
layers{5}.init_type = 'uniform'; % initialization method, Gaussian and uniform are provided.

% Sixth layer is another convolution layer
layers{6}.type = 'CONV';
%layers{2}.num = 20; % # of output channels
layers{6}.k = 3; % kernel size in CeNN is 3X3
%layers{2}.stride = 1; % stride size
%layers{2}.pad = 0; % padding size
layers{6}.group = 1; % group of input feature maps
layers{6}.Btemplate = zeros(3,3);
layers{6}.threshold = 0;

% Seventh layer is the ReLU layer
layers{7}.type = 'RELU';

% Eighth layer is another pooling layer
layers{8}.type = 'POOLING';
layers{8}.act_type = 'MAX';
layers{8}.k = 2;
layers{8}.stride = 2;
layers{8}.pad = 0;

% Ninth layer is the
layers{9}.type = 'LOSS'; % loss layer
layers{9}.num = 10; % number of classes (10 digits)
...
% Third layer is pooling layer  - use twice B templates for ReLU
layers{3}.type = 'POOLING1';
...
layers{3}.Btemplate_1 = zeros(3,3);
layers{3}.threshold_1 = 0;
layers{3}.Btemplate_2 = zeros(3,3);
layers{3}.threshold_2 = 0;
...
for iter = 1 : max_iter
    ...
    [cp, param_grad] = CeNN_net(params, layers, xtrain(:, id), ytrain(id));
